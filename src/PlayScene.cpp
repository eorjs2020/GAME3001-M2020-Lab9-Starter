...
void PlayScene::draw()
{
	...
	if (m_bDebugMode)
	{
		...
		Util::DrawRect(m_pPlaneSprite->getTransform()->position - glm::vec2(m_pPlaneSprite->getWidth() * 0.5f, 
			m_pPlaneSprite->getHeight() * 0.5f), m_pPlaneSprite->getWidth(), m_pPlaneSprite->getHeight());
...
void PlayScene::handleEvents()
{
	...
	if (!m_bDebugKeys[H_KEY])
	{
		if (EventManager::Instance().isKeyDown(SDL_SCANCODE_H))
		{
			...
			if (m_bDebugMode)
				...
				std::cout << "Debubug Mode Off" << std::endl;
		}
	}
	...
	if (!m_bDebugKeys[K_KEY])
	{
		...
		{
			std::cout << "Debug: Enemy Takse Damage!!" << std::endl;
...
{
	m_bPatrolMode = m_bDebugMode = false;
